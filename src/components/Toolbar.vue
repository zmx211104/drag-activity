        ...
        <div class="toolbar">
		 ...
		 <!--       <el-button @click="undo">撤消</el-button>
           ...
           <!-- <el-button style="margin-left: 10px;" @click="preview(false)">预览</el-button> -->
            <!-- <el-button @click="save" style="margin-left:10px">保存画布数据</el-button> -->
           ...
           <!-- <el-button :disabled="!curComponent || curComponent.isLock" @click="lock">锁定</el-button>
            ...
            <div class="canvas-config">
			  ...
			  <el-button @click="save" type="primary" style="margin-left:20px;">保存画布数据</el-button>
        </div>
...
export default {
    ...
    methods: {
        ...
        save() {
		  ...
		  console.log("this.canvasStyleData.backgroundImg===",this.canvasStyleData.backgroundImg)
            localStorage.setItem('canvasData', JSON.stringify(this.componentData))
            localStorage.setItem('canvasStyle', JSON.stringify(this.canvasStyleData))
			localStorage.setItem('materialList', JSON.stringify(this.$store.state.materialList))
            //存到缓存
			let savePage = {
				pageUrl:this.GLOBAL.pageUrl,
				pageData:{
					canvasData:JSON.stringify(this.componentData),
					canvasStyle:JSON.stringify(this.canvasStyleData),
					materialList:JSON.stringify(this.$store.state.materialList)
				}
			}
			window.parent.postMessage(savePage, "*");
        ...
        clearCanvas() {
			...
			this.$store.commit('setCanvasImg','')
            this.$store.commit('recordSnapshot')
			//将背景图素材置为空
			//materialList
			this.$store.state.materialList =[];
        },
